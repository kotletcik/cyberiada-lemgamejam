...
var current_dance_movement_speed
...
func give_action_to_aliens():
	for alien in aliens:
		...
		var alien_target_pos_index = ((alien.id + 1) + Cycle_manager.instance.general_step_counter * current_dance_movement_speed) % current_aliens_path.size()
		alien.current_alien_action = Alien_action.new(current_aliens_path[alien_target_pos_index], current_dance_name)
		alien.speed = current_dance_movement_speed
...
func change_dance_randomly():
	...
	current_dance_movement_speed = new_dance.moving_speed
